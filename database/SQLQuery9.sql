...
CREATE TABLE RETURN1 ( -- Using RETURN1 to avoid conflict with reserved keyword RETURN
    ...
    order_id INT NOT NULL FOREIGN KEY REFERENCES [ORDER](order_id),
    reason_id INT NOT NULL FOREIGN KEY REFERENCES RETURN_REASON(reason_id), -- Retained NOT NULL as per user's schema
    return_date DATE
);
...
SELECT TOP 10 * FROM CUSTOMER_Staging;
SELECT TOP 10 * FROM LOCATION;

SELECT COUNT(*) AS RowsWithBadRegisterDate
FROM CUSTOMER_Staging
WHERE TRY_CONVERT(DATE, register_date, 103) IS NULL;

SELECT COUNT(*) AS RowsWithMissingLocationID
FROM CUSTOMER_Staging cs
WHERE NOT EXISTS (SELECT 1 FROM LOCATION l WHERE l.location_id = cs.location_id);

SELECT COUNT(*) AS RowsWithMissingMarketingChannelID
FROM CUSTOMER_Staging cs
WHERE cs.marketing_channel_id IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM MARKETING_CHANNEL mc WHERE mc.marketing_channel_id = cs.marketing_channel_id);