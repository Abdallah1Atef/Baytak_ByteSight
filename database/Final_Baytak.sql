...
SELECT
    t.name AS TableName,
    SUM(p.rows) AS RowCount1
FROM
    sys.tables t
INNER JOIN
    sys.partitions p ON t.object_id = p.object_id
WHERE
    p.index_id < 2 -- 0 = Heap, 1 = Clustered Index
...
(104, CONVERT(DATE, '10/1/2024', 101), CONVERT(DATE, '1/1/2025', 101), REPLACE('150,000', ',', '')),
...
(107, CONVERT(DATE, '5/30/2025', 101), CONVERT(DATE, '8/30/2025', 101), REPLACE('240,000', ',', ''));
GO
--BULK INSERT MARKETING
...
WHERE
    TABLE_CATALOG = 'Baytak' -- Specify your database name
    AND DATA_TYPE IN ('date', 'datetime', 'datetime2', 'smalldatetime', 'time', 'datetimeoffset')
ORDER BY
