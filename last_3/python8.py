...
MARKETING_DATA = [
    {"Campaign_Id": 101, "start": datetime(2024, 1, 1, 9, 0), "end": datetime(2024, 4, 1, 9, 0), "market_cost": 60000},
    {"Campaign_Id": 102, "start": datetime(2024, 4, 10, 8, 0), "end": datetime(2024, 7, 10, 8, 0), "market_cost": 100000},
    {"Campaign_Id": 103, "start": datetime(2024, 7, 20, 10, 0), "end": datetime(2024, 10, 20, 10, 0), "market_cost": 80000},
    {"Campaign_Id": 104, "start": datetime(2024, 10, 1, 18, 0), "end": datetime(2025, 1, 1, 18, 0), "market_cost": 140000},
    {"Campaign_Id": 105, "start": datetime(2025, 1, 15, 0, 0), "end": datetime(2025, 4, 15, 0, 0), "market_cost": 120000},
    {"Campaign_Id": 106, "start": datetime(2025, 4, 25, 12, 0), "end": datetime(2025, 7, 25, 12, 0), "market_cost": 160000},
    {"Campaign_Id": 107, "start": datetime(2025, 5, 30, 9, 30), "end": datetime(2025, 8, 30, 9, 30), "market_cost": 200000},
]
...
def calculate_daily_activity_weights(start_date, end_date, branches_data, marketing_data):
    ...
    while current_date <= end_date:
        ...
        # Impact of Branch Openings (increased weight)
        for branch in branches_data:
            if current_date.date() >= branch['opening_date'].date():
                weight += 2.0 # Each open branch adds significantly to activity weight

        # Impact of Marketing Campaigns (dramatically increased weight)
        for campaign in marketing_data:
            if campaign['start'].date() <= current_date.date() <= campaign['end'].date():
                weight += 10.0 # Each active campaign adds a very high weight
...
def generate_product_table(designs_data):
    ...
    for p_name, p_category in PRODUCT_NAMES_AND_CATEGORIES.items():
        cost_min, cost_max = PRODUCT_COST_RANGES.get(p_category, (50 * 4, 1000 * 4)) # Multiplied by 4
...
def generate_customer_table(num_customers_target, daily_weights_map, marketing_data):
    customers = []
    customer_id_counter = 1

    dates = list(daily_weights_map.keys())
    
    # Determine the earliest marketing campaign start date
    earliest_campaign_start_date = min(m['start'] for m in marketing_data).date() if marketing_data else datetime.max.date()

    # Calculate daily allocations for new customers
    daily_new_customer_allocations = {date: 0 for date in dates}
    total_daily_weights = sum(daily_weights_map.values())

    if total_daily_weights > 0:
        for date, weight in daily_weights_map.items():
            daily_new_customer_allocations[date] = round(num_customers_target * (weight / total_daily_weights))
    
    # Adjust to ensure exact target is met
    current_customer_sum = sum(daily_new_customer_allocations.values())
    diff_customer = num_customers_target - current_customer_sum
    if diff_customer != 0:
        for date in sorted(daily_new_customer_allocations.keys(), key=lambda d: daily_weights_map[d], reverse=True):
            if diff_customer == 0: break
            if diff_customer > 0:
                daily_new_customer_allocations[date] += 1
                diff_customer -= 1
            else:
                if daily_new_customer_allocations[date] > 0:
                    daily_new_customer_allocations[date] -= 1
                    diff_customer += 1

    # Generate customers based on daily allocations
    for date in sorted(dates): # Process dates chronologically
        num_customers_for_day = daily_new_customer_allocations.get(date, 0)
        for _ in range(num_customers_for_day):
            if customer_id_counter > num_customers_target:
                break # Stop if target is already met

            age = random.randint(18, 35) if random.random() < 0.75 else random.randint(36, 60)
            gender = random.choice(['Male', 'Female'])
            phone = generate_egyptian_phone()
            location_id = random.choice([loc['location_id'] for loc in LOCATION_DATA])
            
            if date < earliest_campaign_start_date:
                channel_id = 0
            else:
                channel_id = random.choice([ch['channel_id'] for ch in CHANNEL_DATA])

            customers.append([
                customer_id_counter, location_id, channel_id, age, gender, phone, date.strftime('%Y-%m-%d')
            ])
            customer_id_counter += 1
        if customer_id_counter > num_customers_target:
            break
...
def generate_leads_table(num_leads_target, daily_weights_map):
    leads = []
    lead_id_counter = 1

    dates = list(daily_weights_map.keys())

    # Calculate daily allocations for new leads
    daily_new_lead_allocations = {date: 0 for date in dates}
    total_daily_weights = sum(daily_weights_map.values())

    if total_daily_weights > 0:
        for date, weight in daily_weights_map.items():
            daily_new_lead_allocations[date] = round(num_leads_target * (weight / total_daily_weights))
    
    # Adjust to ensure exact target is met
    current_lead_sum = sum(daily_new_lead_allocations.values())
    diff_lead = num_leads_target - current_lead_sum
    if diff_lead != 0:
        for date in sorted(daily_new_lead_allocations.keys(), key=lambda d: daily_weights_map[d], reverse=True):
            if diff_lead == 0: break
            if diff_lead > 0:
                daily_new_lead_allocations[date] += 1
                diff_lead -= 1
            else:
                if daily_new_lead_allocations[date] > 0:
                    daily_new_lead_allocations[date] -= 1
                    diff_lead += 1

    # Generate leads based on daily allocations
    for date in sorted(dates): # Process dates chronologically
        num_leads_for_day = daily_new_lead_allocations.get(date, 0)
        for _ in range(num_leads_for_day):
            if lead_id_counter > num_leads_target:
                break # Stop if target is already met
            ...
            leads.append([
                lead_id_counter, phone, gender, date.strftime('%Y-%m-%d')
            ])
            lead_id_counter += 1
        if lead_id_counter > num_leads_target:
            break
...
def generate_order_table(num_orders, customers_data, daily_weights_map):
    orders_raw = []
    # Map customer_id to their SignUpDate (date object)
    customer_signup_dates = {c[0]: datetime.strptime(c[6], '%Y-%m-%d').date() for c in customers_data}

    dates = list(daily_weights_map.keys())
    weights = list(daily_weights_map.values())

    # Calculate daily allocations for orders
    daily_order_allocations = {date: 0 for date in dates}
    total_daily_weights_orders = sum(daily_weights_map.values())

    if total_daily_weights_orders > 0:
        for date, weight in daily_weights_map.items():
            daily_order_allocations[date] = round(num_orders * (weight / total_daily_weights_orders))
    
    # Adjust to ensure exact target is met
    current_order_sum = sum(daily_order_allocations.values())
    diff_order = num_orders - current_order_sum
    if diff_order != 0:
        for date in sorted(daily_order_allocations.keys(), key=lambda d: daily_weights_map[d], reverse=True):
            if diff_order == 0: break
            if diff_order > 0:
                daily_order_allocations[date] += 1
                diff_order -= 1
            else:
                if daily_order_allocations[date] > 0:
                    daily_order_allocations[date] -= 1
                    diff_order += 1

    order_id_counter = 1
    for date in sorted(dates):
        num_orders_for_day = daily_order_allocations.get(date, 0)
        for _ in range(num_orders_for_day):
            if order_id_counter > num_orders:
                break

            available_branches = [b for b in BRANCHES_DATA if b['opening_date'].date() <= date]
            ...
            order_datetime = datetime.combine(date, order_time_of_day)
...
def generate_order_line_table(num_order_lines, orders_data, products_data, discounts_data):
    ...
    for order in sorted_orders:
        ...
        # Each order can have 1 to 3 rows in order line table
        num_unique_products_in_order = random.randint(1, 3) 
        
        remaining_order_lines_needed = num_order_lines - order_line_counter
        if remaining_orders_count := len(sorted_orders) - sorted_orders.index(order): # Walrus operator for Python 3.8+
            avg_lines_per_remaining_order = remaining_order_lines_needed / remaining_orders_count
            num_unique_products_in_order = int(max(1, min(3, round(avg_lines_per_remaining_order)))) # Max 3 items
        ...
        for _ in range(num_unique_products_in_order):
            if not current_available_products:
            ...
            current_available_products.remove(chosen_product_id) # Ensure uniqueness for this order
            ...
            if order_line_counter >= num_order_lines:
                break # Stop if target number of order lines is reached
        if order_line_counter >= num_order_lines:
            break
...
def generate_branch_visits_log_table(num_visits, customers_data, leads_data, daily_weights_map):
    raw_visit_intentions = []
    
    dates = list(daily_weights_map.keys())
    weights = list(daily_weights_map.values())

    # Map customer_id to their RegisterDate (date object) for eligibility check
    customer_registration_dates = {c[0]: datetime.strptime(c[6], '%Y-%m-%d').date() for c in customers_data}
    # Map lead_id to their datetime (date object) for eligibility check
    lead_creation_dates = {l[0]: datetime.strptime(l[3], '%Y-%m-%d').date() for l in leads_data}

    purchased_leads_set = set() # To keep track of leads who have made a purchase

    print(f"  Generating raw visit intentions for {num_visits} visits...")
    progress_interval = max(1, num_visits // 10) # Report progress every 10%

    for i in range(num_visits):
        if (i + 1) % progress_interval == 0:
            print(f"    Generated {i + 1}/{num_visits} raw visit intentions...")

        visit_date_chosen = random.choices(dates, weights=weights, k=1)[0]
        
        # Ensure the selected branch was open on or before the visit date
        available_branches_for_date = [b for b in BRANCHES_DATA if b['opening_date'].date() <= visit_date_chosen]
        if not available_branches_for_date:
            continue # Skip if no branch is open yet for this date
        selected_branch_id = random.choice(available_branches_for_date)['branch_id']

        entering_time_obj = fake.time_object()
        visit_datetime_full = datetime.combine(visit_date_chosen, entering_time_obj)
        leaving_datetime_full = visit_datetime_full + timedelta(minutes=random.randint(30, 240))
        leaving_time_obj = leaving_datetime_full.time()
        ...
        # Collect eligible existing customers/leads based on their registration date
        eligible_existing_customers = [
            cid for cid in customer_registration_dates
            if customer_registration_dates[cid] <= visit_date_chosen
        ]
        eligible_existing_leads = [
            lid for lid in lead_creation_dates
            if lead_creation_dates[lid] <= visit_date_chosen and lid not in purchased_leads_set # Exclude purchased leads
        ]

        # Prioritize assigning a customer (existing) over a lead (existing)
        if eligible_existing_customers and (not eligible_existing_leads or random.random() < 0.7): # 70% chance to pick customer if both exist
            assigned_customer = random.choice(eligible_existing_customers)
        elif eligible_existing_leads:
            assigned_lead = random.choice(eligible_existing_leads)
        else:
            # If no eligible customers or leads, this visit cannot be assigned to anyone, skip.
            continue 

        raw_visit_intentions.append({
            "visit_datetime_full": visit_datetime_full,
            "branch_id": selected_branch_id,
            "entring": entering_time_obj,
            "leaving": leaving_time_obj,
            "purchased": purchased,
            "assigned_customer_id": assigned_customer,
            "assigned_lead_id": assigned_lead
        })

        # --- Lead Purchase Logic (No Conversion, Just Exclusion from future lead visits) ---
        if assigned_lead is not None and purchased is True:
            # If a lead makes a purchase, they are marked as 'purchased' and won't be reused as a lead
            purchased_leads_set.add(assigned_lead)

    # Sort raw visits by their full datetime for chronological ID assignment
    raw_visit_intentions.sort(key=lambda x: x["visit_datetime_full"])

    # Format final visits data for CSV output
    final_visits = []
    for i, visit_data in enumerate(raw_visit_intentions):
        # Ensure customer_id and lead_id are empty strings if None
        customer_id_output = visit_data["assigned_customer_id"] if visit_data["assigned_customer_id"] is not None else ''
        lead_id_output = visit_data["assigned_lead_id"] if visit_data["assigned_lead_id"] is not None else ''

        # Combine visit_date_chosen with leaving_time_obj to form full leaving datetime
        leaving_datetime_full = datetime.combine(visit_data["visit_datetime_full"].date(), visit_data["leaving"])
        ...
        final_visits.append([
            ...
            visit_data["visit_datetime_full"].strftime('%#m/%#d/%Y %H:%M:%S'), # Entering datetime
            leaving_datetime_full.strftime('%#m/%#d/%Y %H:%M:%S'), # Leaving datetime (full date and time)
            'TRUE' if visit_data["purchased"] else 'FALSE' # Format boolean as TRUE/FALSE string
        ])
...
if __name__ == "__main__":
    ...
    NUM_CUSTOMERS_TARGET = 3000 # Target unique customers to generate through visits
    NUM_LEADS_TARGET = 1000     # Target unique leads to generate through visits
    NUM_ORDERS = 10000
    NUM_ORDER_LINES = 25000 # Increased to 25000 as requested
    NUM_BRANCH_VISITS = 15000 # This number now represents the total number of visits, not directly customers/leads
    ...
    # 3. Generate Customer Table (now explicitly generates target number of customers)
    customers_data = generate_customer_table(NUM_CUSTOMERS_TARGET, daily_weights, MARKETING_DATA)
    write_to_csv('customer_table.csv', ["customer_id", "location_id", "ChannelID", "Age", "Gender", "Phone", "RegisterDate"], customers_data)

    # 4. Generate Leads Table (now explicitly generates target number of leads)
    leads_data = generate_leads_table(NUM_LEADS_TARGET, daily_weights)
    write_to_csv('leads_table.csv', ["leads_id", "phone", "gender", "datetime"], leads_data)

    # 5. Generate Branch Visits Log Table (links to pre-generated customers and leads)
    branch_visits_data = generate_branch_visits_log_table(NUM_BRANCH_VISITS, customers_data, leads_data, daily_weights)
    write_to_csv('branch_visits_log_table.csv', ["visit_id", "customer_id", "lead_id", "branch_id", "entring_datetime", "leaving_datetime", "purchased"], branch_visits_data)
    
    # 6. Generate Order Table (now depends on customers_data from pre-generation)
    ...
    # 7. Generate Delivery Table
    deliveries_data = generate_delivery_table(orders_data, num_delayed_deliveries=150)
    write_to_csv('delivery_table.csv', ["Order_id", "scheduled_deliver_date", "deliver_date"], deliveries_data)

    # 8. Generate Discount Table
    ...
    marketing_header = ["Campaign_Id", "start", "end", "market_cost"]
    marketing_rows = [[m["Campaign_Id"], m["start"].strftime('%Y-%m-%d %H:%M:%S'), m["end"].strftime('%Y-%m-%d %H:%M:%S'), m["market_cost"]] for m in MARKETING_DATA]
