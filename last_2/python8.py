...
# Removed generate_customer_table and generate_leads_table as they are now integrated into branch visits
...
def generate_order_table(num_orders, customers_data, daily_weights_map):
    ...
    for _ in range(num_orders):
        ...
        # Select a customer whose signup date is on or before the current order date
        eligible_customers = [
...
def generate_branch_visits_log_table(num_visits, num_customers_target, num_leads_target, daily_weights_map, marketing_data):
    ...
    for i in range(num_visits):
        ...
        raw_visit_intentions.append({
            ...
            "purchased": purchased,
            "assigned_customer_id": None, # Will be assigned in next phase
            "assigned_lead_id": None      # Will be assigned in next phase
        })
    ...
    customers_data_generated = []
    leads_data_generated = []
    customer_id_counter = 1
    lead_id_counter = 1
    
    # Maps to store details of generated customers/leads, including their registration date (as date object)
    customer_details_map = {} # {id: {RegisterDate_date_obj, location_id, channel_id, Age, Gender, Phone}}
    lead_details_map = {}     # {id: {datetime_date_obj, phone, gender}}
    converted_leads_set = set() # To keep track of leads that have converted to customers

    print("  Assigning customer/lead IDs to visits chronologically...")
    for i, raw_visit in enumerate(raw_visit_intentions):
        if (i + 1) % progress_interval == 0:
            print(f"    Processing {i + 1}/{len(raw_visit_intentions)} visits for ID assignment...")

        visit_date_chosen = raw_visit["visit_datetime_full"].date()
        ...
        eligible_existing_customers = [
            cid for cid, details in customer_details_map.items()
            if details["RegisterDate_date_obj"] <= visit_date_chosen
        ]
        eligible_existing_leads = [
            lid for lid, details in lead_details_map.items()
            if details["datetime_date_obj"] <= visit_date_chosen and lid not in converted_leads_set # Exclude converted leads
        ]

        possible_assignments = []

        # Option 1: Create a new customer if target not met (higher priority)
        if customer_id_counter <= num_customers_target:
            possible_assignments.extend(["new_customer"] * 3) # Give new customers more weight
        # Option 2: Use an existing customer if available (higher priority)
        if eligible_existing_customers:
            possible_assignments.extend(["existing_customer"] * 5) # Give existing customers even more weight
        
        # Option 3: Create a new lead if target not met
        if lead_id_counter <= num_leads_target:
            possible_assignments.append("new_lead")
        # Option 4: Use an existing lead if available
        if eligible_existing_leads:
            possible_assignments.append("existing_lead")

        if not possible_assignments:
            # If no possible assignments, try to force a new customer or lead if targets allow
            if customer_id_counter <= num_customers_target:
                chosen_assignment_type = "new_customer"
            elif lead_id_counter <= num_leads_target:
                chosen_assignment_type = "new_lead"
            else:
                continue # Still no one can be assigned, skip this visit
        else:
            chosen_assignment_type = random.choice(possible_assignments)

        if chosen_assignment_type == "new_customer":
            assigned_customer = customer_id_counter
            customer_id_counter += 1
            ...
            # Set channel_id to 0 if RegisterDate is before earliest marketing campaign
            if visit_date_chosen < earliest_campaign_start_date:
                channel_id = 0
            else:
                channel_id = random.choice([ch['channel_id'] for ch in CHANNEL_DATA])

            customer_details_map[assigned_customer] = {
                "RegisterDate_date_obj": visit_date_chosen,
            ...
            customers_data_generated.append([
                assigned_customer, location_id, channel_id, age, gender, phone, visit_date_chosen.strftime('%Y-%m-%d')
            ])
        elif chosen_assignment_type == "existing_customer":
            assigned_customer = random.choice(eligible_existing_customers)
        elif chosen_assignment_type == "new_lead":
            assigned_lead = lead_id_counter
            lead_id_counter += 1

            phone = generate_egyptian_phone()
            gender = random.choice(['Male', 'Female'])
            
            lead_details_map[assigned_lead] = {
                "datetime_date_obj": visit_date_chosen,
                "phone": phone,
                "gender": gender,
            }
            leads_data_generated.append([
                assigned_lead, phone, gender, visit_date_chosen.strftime('%Y-%m-%d')
            ])
        elif chosen_assignment_type == "existing_lead":
            assigned_lead = random.choice(eligible_existing_leads)
        
        raw_visit["assigned_customer_id"] = assigned_customer
        raw_visit["assigned_lead_id"] = assigned_lead

        # --- Lead to Customer Conversion Logic ---
        if raw_visit["assigned_lead_id"] is not None and raw_visit["purchased"] is True:
            converted_lead_id = raw_visit["assigned_lead_id"]
            lead_info = lead_details_map[converted_lead_id]

            # Generate new customer details for the converted lead
            new_customer_id = customer_id_counter
            customer_id_counter += 1
            
            # Generate new customer-specific attributes for the converted lead
            age = random.randint(18, 35) if random.random() < 0.75 else random.randint(36, 60)
            location_id = random.choice([loc['location_id'] for loc in LOCATION_DATA])
            ...
            customer_details_map[new_customer_id] = {
                "RegisterDate_date_obj": visit_date_chosen, # Registration date is the conversion date
                "location_id": location_id,
                "channel_id": channel_id,
                "Age": age,
                "Gender": lead_info["gender"], # Keep gender from lead
                "Phone": lead_info["phone"],   # Keep phone from lead
            }
            customers_data_generated.append([
                new_customer_id, location_id, channel_id, age, lead_info["gender"], lead_info["phone"], visit_date_chosen.strftime('%Y-%m-%d')
            ])

            # Update the current raw_visit to reflect the conversion
            raw_visit["assigned_customer_id"] = new_customer_id
            raw_visit["assigned_lead_id"] = None
            
            # Add the original lead_id to the set of converted leads
            converted_leads_set.add(converted_lead_id)
    ...
    for i, visit_data in enumerate(raw_visit_intentions):
        ...
        final_visits.append([
            ...
            # Format entering and leaving as full datetime
            visit_data["visit_datetime_full"].strftime('%#m/%#d/%Y %H:%M:%S'), # Entering datetime
            visit_data["leaving"].strftime('%#m/%#d/%Y %H:%M:%S'), # Leaving datetime (using leaving time object combined with visit date)
            'TRUE' if visit_data["purchased"] else 'FALSE' # Format boolean as TRUE/FALSE string
        ])
    
    # Sort generated customers and leads by their IDs to ensure chronological order for CSV output
    customers_data_generated.sort(key=lambda x: x[0])
    leads_data_generated.sort(key=lambda x: x[0])

    return final_visits, customers_data_generated, leads_data_generated
...
if __name__ == "__main__":
    ...
    # 3. Generate Branch Visits Log Table (which now also generates customers and leads)
    branch_visits_data, customers_data, leads_data = generate_branch_visits_log_table(NUM_BRANCH_VISITS, NUM_CUSTOMERS_TARGET, NUM_LEADS_TARGET, daily_weights, MARKETING_DATA)
    write_to_csv('branch_visits_log_table.csv', ["visit_id", "customer_id", "lead_id", "branch_id", "entring", "leaving", "purchased"], branch_visits_data)
    
    # 4. Write Customer Table (generated from branch visits)
    write_to_csv('customer_table.csv', ["customer_id", "location_id", "ChannelID", "Age", "Gender", "Phone", "RegisterDate"], customers_data)

    # 5. Write Leads Table (generated from branch visits)
    write_to_csv('leads_table.csv', ["leads_id", "phone", "gender", "datetime"], leads_data)

    # 6. Generate Order Table (now depends on customers_data from branch visits)
